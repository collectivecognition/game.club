...
describe('BeerListContainer', () => {
  ...
  it('should start with an empty list', () => {
    const wrapper = shallow(<BeerListContainer/>);
    expect(wrapper.state('beers').equals(List())).to.equal(true);
  });

  it('adds items to the list', () => {
    const wrapper = shallow(<BeerListContainer/>);
    wrapper.instance().addItem('Sam Adams');
    expect(wrapper.state('beers').equals(List(['Sam Adams']))).to.equal(true);
  });
...
describe('InputArea', () => {
  ...
  it('should clear the input when an item is added', () => {
    const addItemSpy = spy();
    const wrapper = shallow(<InputArea onSubmit={addItemSpy}/>);
    const input = wrapper.find('input');
    const addButton = wrapper.find('button');
    input.simulate('change', {target: {value: 'Budweiser'}});
